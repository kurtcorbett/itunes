    ...
    app.controller('mainCtrl', function ($scope, itunesService) {
      ...
      //this means when you make your iTunes request, you'll need to get back the information, parse it accordingly, then set it to songData on the scope -> $scope.songData = ...
        ...
        $scope.gridOptions = {
            ...
            sortInfo: { fields: ['Song', 'Artist', 'Collection', 'Type'], directions: ['asc'] },
            columnDefs: [
                { field: 'Play', displayName: 'Play', width: '40px', cellTemplate: '<a href="{{row.getProperty(col.field)}}"><img ng-src="http://www.icty.org/x/image/Miscellaneous/play_icon30x30.png"></a>' },
                { field: 'Artist', displayName: 'Artist' },
                { field: 'Collection', displayName: 'Collection' },
                { field: 'AlbumArt', displayName: 'Album Art', width: '110px', cellTemplate: '<img ng-src="{{row.getProperty(col.field)}}">' },
                { field: 'Type', displayName: 'Type' },
                { field: 'CollectionPrice', displayName: 'Collection Price' }
            ]
        };
